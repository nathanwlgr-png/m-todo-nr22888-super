...
IMPORTANTE: Resposta COMPLETA, nunca corte no meio. Use markdown estruturado.`
  ...
  const generateQuickAction = async (type) => {
    ...
    try {
      ...
      const ctx = `Cliente: ${client.first_name} | Clínica: ${client.clinic_name || ''} | Cidade: ${client.city || ''} | Tipo: ${client.client_type || ''} | Numerologia ${client.numerology_number}: ${client.behavioral_profile || ''} | Tom: ${client.client_tone || ''} | Score: ${client.purchase_score}% | Status: ${client.status} | Dores: ${client.main_pains?.join(', ') || ''} | Equip atual: ${client.current_equipment || ''} | Volume: ${client.current_volume || ''} | Orçamento: R$${client.available_budget || ''} | Prazo: ${client.decision_deadline || ''} | Visitas: ${visits.length} | Vendas: ${sales.length} | Gatilhos já usados: ${client.triggers_used?.join(', ') || ''} | Objeções reais: ${client.real_objections?.join(', ') || ''} | Motivadores: ${client.purchase_motivators?.join(', ') || ''} | Melhor horário IA: ${client.ai_sales_intelligence?.optimal_contact_time || ''} | LTV est: R$${client.ai_sales_intelligence?.ltv_12_months || ''}/12m`;

      const prompts = {
        presentation: `MÉTODO NR22 — GUIA COMPLETO DE APRESENTAÇÃO\n\n${ctx}\n\nCrie um roteiro COMPLETO e PERSONALIZADO para ${client.first_name}:\n1. Abertura baseada no perfil numerológico ${client.numerology_number} (tom exato)\n2. Apresentação presencial (script palavra por palavra)\n3. Versão WhatsApp (mensagem pronta)\n4. Versão ligação (roteiro de 2 min)\n5. Diferenciação vs concorrentes (25m garantia, manutenção vitalícia, bonificação insumos, ISO 13485)\n6. Checklist de preparação pré-visita\n7. Frase filosófica motivacional de Napoleão Hill para Nathan\n\nResposta COMPLETA em markdown profissional.`,

        insights: `MÉTODO NR22 — ANÁLISE PSICOLÓGICA E ESTRATÉGICA PROFUNDA\n\n${ctx}\n\nGere análise COMPLETA:\n1. Perfil psicológico detalhado (numerologia ${client.numerology_number} + tom + estilo)\n2. Motivadores de compra reais (conscientes e inconscientes)\n3. Medos e resistências prováveis\n4. Canal e horário ideal de abordagem (com justificativa)\n5. Gatilhos Cialdini mais efetivos para este perfil (com script)\n6. Estratégia de pitch em 3 atos\n7. Alertas de risco (o que NUNCA dizer)\n8. Probabilidade de conversão e próximos 3 passos\n9. Frase motivacional de Platão para Nathan\n\nResposta COMPLETA em markdown.`,

        prospecting: `MÉTODO NR22 — ESTRATÉGIA DE PROSPECÇÃO\n\n${ctx}\n\nCrie plano COMPLETO de prospecção:\n1. Análise do momento atual do cliente (ciclo numerológico + score)\n2. Canal ideal (WhatsApp/telefone/email/visita) com justificativa\n3. Horário e frequência recomendados\n4. Primeira frase de abertura (adaptada ao perfil ${client.numerology_number})\n5. Sequência de 5 contatos (com intervalos e canais)\n6. Scripts para cada canal (palavra por palavra)\n7. Como lidar se não responder\n8. Frase motivacional de Sócrates para Nathan`,

        question: `MÉTODO NR22 — PERGUNTAS SPIN SELLING\n\n${ctx}\n\nGere sequência COMPLETA SPIN para ${client.first_name}:\n\n**SITUAÇÃO (S):** 3 perguntas para mapear contexto atual\n**PROBLEMA (P):** 3 perguntas para revelar dores ocultas\n**IMPLICAÇÃO (I):** 3 perguntas para amplificar o custo da inação\n**NECESSIDADE-SOLUÇÃO (N):** 3 perguntas para o cliente concluir que precisa da solução\n\nAdapte cada pergunta ao perfil numerológico ${client.numerology_number} e tom ${client.client_tone}.\nIndique qual técnica Cialdini usar após cada resposta.\nResposta COMPLETA em markdown.`,

        objection: `MÉTODO NR22 — CONTROLE MAESTRAL DE OBJEÇÕES\n\n${ctx}\n\nGere respostas COMPLETAS para as objeções mais prováveis:\n1. "Está caro / não tenho orçamento" → resposta + técnica usada\n2. "Preciso pensar / consultar sócio" → resposta + urgência\n3. "Meu equipamento ainda funciona" → resposta + comparativo\n4. "Já tenho IDEXX/Mindray/Mobivet" → resposta + diferenciação\n5. "Não tenho volume suficiente" → resposta + cálculo ROI\n6. Objeção específica de ${client.real_objections?.join(' / ') || 'preço'}\n\nPara cada: frase exata de resposta + framework usado (SPIN/Cialdini/Value Selling) + gatilho mental.\nAdapte ao perfil numerológico ${client.numerology_number}.\nResposta COMPLETA em markdown.`,

        closing: `MÉTODO NR22 — TÉCNICAS DE FECHAMENTO\n\n${ctx}\n\nGere arsenal COMPLETO de fechamento para ${client.first_name}:\n1. Frase de fechamento principal (adaptada ao numerológico ${client.numerology_number})\n2. Fechamento por escassez (baseado em Cialdini)\n3. Fechamento por resumo de valor (ROI calculado)\n4. Fechamento condicional ("Se eu conseguir X, você fecha?") \n5. Script para superar última resistência\n6. Como lidar com "deixa eu pensar mais"\n7. CTA final com urgência real\n8. Frase motivacional de Napoleão Hill sobre perseverança`,

        followup: `MÉTODO NR22 — MENSAGEM DE FOLLOW-UP\n\n${ctx}\n\nGere 3 versões de follow-up para ${client.first_name}:\n\n**VERSÃO 1 — WhatsApp curta (max 3 linhas):**\n[texto pronto para copiar]\n\n**VERSÃO 2 — WhatsApp média (personalizada com dados):**\n[texto com ROI e próximo passo]\n\n**VERSÃO 3 — Email profissional:**\n[assunto + corpo completo]\n\nAdapte ao perfil ${client.numerology_number} e inclua: referência ao último contato, valor específico, CTA claro com data.\nResposta COMPLETA.`,

        proposal: `MÉTODO NR22 — PROPOSTA COMERCIAL PERSONALIZADA\n\n${ctx}\n\nGere proposta COMPLETA para ${client.first_name}:\n\n# PROPOSTA COMERCIAL — SEAMATY\n**Cliente:** ${client.first_name} | **Clínica:** ${client.clinic_name || 'N/A'} | **Data:** ${new Date().toLocaleDateString('pt-BR')}\n\n1. Abertura personalizada (tom numerológico ${client.numerology_number})\n2. Diagnóstico das necessidades identificadas\n3. Equipamento recomendado com especificações\n4. Cálculo de ROI (volume ${client.current_volume} × economia mensal)\n5. Condições comerciais (à vista ou 5x cartão s/juros)\n6. Bonificação em insumos (nunca desconto no equipamento)\n7. Diferenciais: 25m garantia, manutenção vitalícia, ISO 13485\n8. Cases similares de sucesso\n9. CTA com urgência (sem ser forçado)\n10. Assinatura Nathan Rosa — CMAT Brasil\n\nResposta COMPLETA e pronta para WhatsApp.`,

        needs: `MÉTODO NR22 — ANÁLISE PREDITIVA DE NECESSIDADES\n\n${ctx}\n\nRealiza análise COMPLETA de necessidades futuras:\n1. Mapeamento atual vs ideal (gap de soluções)\n2. Próximo produto com maior probabilidade de compra (com %)\n3. Dores ainda não exploradas\n4. Janela de oportunidade (timing baseado em ciclo numerológico + score)\n5. Estratégia de upsell (produto atual → próximo)\n6. Oportunidades de cross-sell identificadas\n7. Valor de lifetime (LTV) estimado 12/24/36 meses\n8. Alerta de risco de perda para concorrente\n9. Gatilho recomendado para ativação agora\n10. Plano de nurturing se ainda não pronto para comprar`,

        suggestTasks: `MÉTODO NR22 — PLANO DE AÇÕES CONCRETAS\n\n${ctx}\n\nCrie plano de ação COMPLETO com 5-7 tarefas para ${client.first_name}:\n\nFormato para cada tarefa:\n**[NÚMERO]. [TIPO EM MAIÚSCULAS] — [Título claro]**\n• Prioridade: ALTA/MÉDIA/BAIXA\n• Prazo: X dias\n• Canal: WhatsApp/Telefone/Email/Visita\n• Script/Descrição: [o que exatamente fazer e dizer]\n• Objetivo: [resultado esperado]\n\nOrdenar por impacto × urgência. Incluir pelo menos:\n- 1 tarefa de contato imediato\n- 1 tarefa de envio de material\n- 1 tarefa de agendamento\n- 1 tarefa de follow-up\n\nResposta COMPLETA em markdown.`,
      };
