          ...
          {formData.steps.map((step, index) => (
            <Card key={index} className="p-4 mb-3">
              ...
              <div className="space-y-3">
                ...
                {(step.channel === 'email' || step.channel === 'whatsapp') && (
                  <div>
                    <Label>{step.channel === 'email' ? 'Assunto do Email' : 'Título da Mensagem'}</Label>
                    <Input
                      value={step.subject}
                      onChange={(e) => updateStep(index, 'subject', e.target.value)}
                      placeholder={step.channel === 'email' ? "Ex: Seguimento da nossa conversa" : "Ex: Continuando nossa conversa"}
                    />
                  </div>
                )}
