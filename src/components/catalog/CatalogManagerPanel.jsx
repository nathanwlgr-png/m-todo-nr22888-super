...
export default function CatalogManagerPanel() {
  ...
  const update = useMutation({ mutationFn: ({ id, data, source }) => base44.functions.invoke('adminProductChange', { entity: 'ProductCatalog', id, data, source, confirmed: true }), onSuccess: () => { client.invalidateQueries({ queryKey: ['product-catalog'] }); toast.success('Catálogo atualizado com auditoria'); }, onError: (error) => toast.error(error.message) });
